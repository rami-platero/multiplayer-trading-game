...
export const checkTrade = async (
  ...
  try {
    ...
    if (!offer) {
      throw new Error(JSON.stringify({ message: "Offer does not exist." }));
    }
    ...
    if (!seller) {
      throw new Error(
        JSON.stringify({ message: "This offer has no trader to trade with." })
      );
    }
    ...
    if (!sellerItem) {
      ...
      throw new Error(
        JSON.stringify({ message: "Selling item does not exist" })
      );
    }
    emitProgress(10);
    ...
    if (filteredItems.length) {
      const buyerItems = filteredItems.every((item: IInventory) => {
        return buyer?.items?.some((buyerItem) => {
          return (
            item.itemId.toString() === buyerItem.itemId.toString() &&
            item.count <= buyerItem.count
          );
        });
      });
      if (!buyerItems) {
        ...
        throw new Error(
          JSON.stringify({
            message: "Buyer does not have every item that was offered",
          })
        );
      }
    }
    ...
    if (buyer?.coins! < offer.coins) {
      io.to(buyer?.socketID).emit("TRADE:ERROR");
      throw new Error(
        JSON.stringify({ message: "Buyer does not have enough coins to trade" })
      );
    }
    emitProgress(20);
